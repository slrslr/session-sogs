...
FOR EACH ROW
WHEN (
NEW.recipient NOT IN (SELECT user FROM user_permissions WHERE admin = 1)
AND NEW.sender NOT IN (SELECT user FROM user_permissions WHERE admin = 1)
...
SELECT RAISE(FAIL, 'Insert operation not allowed for non-admin users.');
-- SELECT RAISE(IGNORE);
END;
